"use client";

import { AiOutlineMenu } from "react-icons/ai";
import Avatar from "../Avatar";
import { useCallback, useState } from "react";
import MenuItem from "./MenuItem";
import useRegisterModal from "@/app/hooks/useRegisterModal";
import useLoginModal from "@/app/hooks/useLoginModal";
import { signOut } from "next-auth/react";
import { SafeUser } from "@/app/types";
import useExhibitModal from "@/app/hooks/useExhibitModal";
import { useRouter } from "next/navigation";

interface UserMenuProps {
  currentUser?: SafeUser | null;
}


const UserMenu: React.FC<UserMenuProps> = ({
  currentUser
}) => {

  const router = useRouter();
  const registerModal = useRegisterModal();
  const loginModal = useLoginModal();
  const exhibitModal = useExhibitModal();

  const [isOpen, setIsOpen] = useState(false);
  const toggleOpen = useCallback(() => {
    setIsOpen((value) => !value);
  }, []);

  const onExhibit = useCallback(() => {
    if (!currentUser) {
      return loginModal.onOpen();
    }
    exhibitModal.onOpen();
  }, [loginModal, currentUser, exhibitModal]);


  return (
    <div className="relative">
      <div className="flex flex-row items-center gap-3">
        <div
          onClick={onExhibit}
          className="md:block text-sm font-semibold py-3 px-4 rounded-full hover:bg-neutral-100
          transition cursor-pointer">
          出品する
        </div>
        <div
          onClick={toggleOpen}
          className="p-4 md:py-1 md:px-2 border-[1px] border-neutral-200 flex flex-row items-center gap-3
          rounded-full cursor-pointer hover:shadow-md transition">
          <AiOutlineMenu />
          <div className="md:block">
            <Avatar src={currentUser?.image} />
          </div>
        </div>
      </div>

      {isOpen && (
        <div className="absolute rounded-xl shadow-md w-[40vw] md:w-3/4 bg-white overflow-hidden
              right-0 top-12 text-sm">
          <div className="flex flex-col cursor-pointer">
            {currentUser ? (
              <>
                <MenuItem
                  onClick={() => router.push('/favorites')}
                  label="お気に入り"
                />
                <MenuItem
                  onClick={() => router.push('/items')}
                  label="購入履歴"
                />
                <MenuItem
                  onClick={() => router.push('/properties')}
                  label="出品リスト"
                />
                <MenuItem
                  onClick={exhibitModal.onOpen}
                  label="出品する"
                />
                <hr />
                <MenuItem
                  onClick={() => signOut()}
                  label="ログアウト"
                />
              </>) : (
              <>
                <MenuItem
                  onClick={loginModal.onOpen}
                  label="ログイン"
                />
                <MenuItem
                  onClick={registerModal.onOpen}
                  label="会員登録"
                />
              </>
            )}
          </div>
        </div>
      )}
    </div>
  )
}

export default UserMenu
